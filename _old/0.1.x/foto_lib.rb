...
# !!! DEPRECATED *** Read input params
def read_input_params
  #utility sub
  def find_first_yaml_file(dir_to_process, mask="*.{yml,yaml}")
  ...
  yaml_mask = "*event*.{yml,yaml}"
  yaml_name = ARGV[1]||find_first_yaml_file(dir_to_process, yaml_mask)

  fail("- no YAML File found (#{yaml_mask});") if yaml_name.nil?  
  fail("- '{#yaml_name}' is not a file;") unless File.file?(yaml_name)
  $log.info "YAML Profile to be processed: #{yaml_name}"
  return [dir_to_process, yaml_name]
end

# Find yaml file in assets directories
def get_1st_yaml ypath=["."], ymask=""
  fmask = []
  ypath.each {|d| fmask << File.join(d, "#{ymask}.{yaml,yml}")}
  $log.info "* Trying to find YAML in #{fmask}"
  Dir.glob(fmask, File::FNM_CASEFOLD)[0] # return 1st found file
end    
...
class FotoEvent
  ...
  def initialize(yaml_config, yaml_event, dir_to_process=File.pwd)
    ...
    # read from config
    fail("- no YAML File defined (#{yaml_config});") if yaml_config.nil?  
    fail("- '{#yaml}' is not a file;") unless File.file?(yaml_config)
    begin   
      yaml = yaml_config
      options_cfg = YAML.load_file(yaml)
      $log.info "YAML loaded: #{yaml}"
    rescue StandardError => e
      raise FatalError, e.full_message(" - Processing YAML #{yaml}; "), e.backtrace
    end

    # read from event profile
    fail("- no YAML File defined (#{yaml_event});") if yaml_event.nil?  
    fail("- '{#yaml}' is not a file;") unless File.file?(yaml_event)
    begin   
    ...
    if alias_creator_copyright.empty?
      ...
      #will read data from creators.yml
      begin
        yaml = get_yaml_from_assets "creator"
        fail(FatalError, "No YAML for creators found; ") if yaml.nil?
        ...
        raise FatalError, e.full_message(" - Parsing YAML #{yaml}; "), e.backtrace
      end
    end
    ...
    if alias_place_created.empty?
      ...
      #will read data from locations.yml
      begin
        yaml = get_yaml_from_assets "place"
        fail(FatalError, "No YAML for places found; ") if yaml.nil?
  ...
  # Find yaml file in assets directories
  def get_yaml_from_assets ymask
    fmask = []
    @dir_assets.each {|d| fmask << File.join(d, "*#{ymask}*.{yaml,yml}")}
    $log.info "* Trying to find YAML in #{fmask}"
    Dir.glob(fmask, File::FNM_CASEFOLD)[0] # return 1st found file
  end    
